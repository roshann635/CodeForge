...
const PROBLEMS = [
  ...
  {
    id: 2, title: "Binary Search", difficulty: "Easy", tags: ["Array", "Binary Search"],
  ...
  {
    id: 4, title: "Reverse Linked List", difficulty: "Medium", tags: ["Linked List"],
  ...
  {
    id: 5, title: "Maximum Subarray", difficulty: "Medium", tags: ["Array", "DP"],
  ...
  {
    id: 6, title: "Merge Two Sorted Lists", difficulty: "Easy", tags: ["Linked List", "Recursion"],
  ...
  {
    id: 7, title: "Climbing Stairs", difficulty: "Easy", tags: ["DP", "Math"],
  ...
  {
    id: 8, title: "Number of Islands", difficulty: "Hard", tags: ["Graph", "BFS", "DFS"],
...
export default function Practice() {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/code/run`, {
        ...
        body: JSON.stringify({ code, language, problemId: problem.id, testCase: tc }),
      });
      ...
      setResults({ mode: "run", status: "error", error: "Backend unreachable. Ensure server is running." });
    }
    setIsRunning(false);
  };
  ...
  const submitCode = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/code/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code, language, problemId: problem.id, testCases: problem.testCases }),
      });
      const data = await res.json();
      ...
      }).then(r => r.json()).then(review => setCodeReview(review)).catch(() => {});
      ...
      setResults({ mode: "submit", status: "error", error: "Backend unreachable." });
    }
  ...
  const analyzeTranscript = async () => {
    try {
      ...
      setVoiceResults({ feedback: "Backend unavailable for analysis.", score: 0 });
    }
  };
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex gap-3 text-white">
      ...
      <div className="w-52 flex-shrink-0 flex flex-col gap-1 overflow-y-auto py-1 pr-1">
        <h3 className="text-xs text-gray-500 uppercase tracking-wider font-medium px-2 mb-2">Problems</h3>
        {PROBLEMS.map((p) => (
          <button key={p.id} onClick={() => setSelectedId(p.id)}
            ...
            <div className="flex items-center justify-between">
              <span className="truncate font-medium">{p.id}. {p.title}</span>
            </div>
            <span className={`text-[10px] font-medium ${p.difficulty === "Easy" ? "text-neon-green" : p.difficulty === "Medium" ? "text-neon-yellow" : "text-neon-magenta"}`}>
              {p.difficulty}
            </span>
          </button>
        ))}
      ...
      <div className="w-80 flex-shrink-0 glass-panel p-5 overflow-y-auto">
        <h2 className="text-xl font-bold font-orbitron text-neon-cyan mb-3">{problem.id}. {problem.title}</h2>
        <div className="flex gap-2 mb-4 flex-wrap">
          <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${diffColors[problem.difficulty]}`}>{problem.difficulty}</span>
          {problem.tags.map((tag, i) => (
            <span key={i} className="text-[10px] px-2 py-0.5 bg-dark-700 text-gray-300 rounded-full border border-dark-600 flex items-center gap-1">
              <Tag size={8} />{tag}
            </span>
          ))}
        </div>
        <div className="space-y-4 text-sm">
          ...
          {problem.examples.map((ex, i) => (
            <div key={i} className="bg-dark-900 border border-dark-700 p-3 rounded-lg space-y-1">
              <p className="font-bold text-gray-200 text-xs">Example {i + 1}:</p>
              <p className="text-gray-400 text-xs"><strong className="text-gray-300">Input:</strong> {ex.input}</p>
              <p className="text-gray-400 text-xs"><strong className="text-gray-300">Output:</strong> {ex.output}</p>
              {ex.explanation && <p className="text-gray-500 text-xs"><strong>Explanation:</strong> {ex.explanation}</p>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        <div className="glass-panel overflow-hidden flex-1 flex flex-col relative">
          ...
          <div className="flex-1">
            <Editor height="100%" theme="vs-dark" language={langMap[language]} value={code}
              onChange={(val) => setCode(val || "")}
              options={{ minimap: { enabled: false }, fontSize: 14, fontFamily: "Fira Code, monospace", padding: { top: 12 }, scrollBeyondLastLine: false }} />
          </div>
        </div>

        {/* Console + Actions */}
        <div className={`glass-panel p-3 flex flex-col gap-3 border border-dark-700 transition-all ${showVoice ? "h-auto" : "max-h-[300px]"}`}>
          <div className="flex items-center justify-between">
            <h3 className="font-orbitron text-gray-400 text-xs uppercase tracking-wider">Terminal</h3>
            <div className="flex gap-2">
              <button onClick={runCode} disabled={isRunning || isSubmitting}
                ...
                {isRunning ? <Loader size={14} className="animate-spin" /> : <Play size={14} />} {isRunning ? "Running..." : "Run"}
              </button>
              <button onClick={submitCode} disabled={isRunning || isSubmitting}
                ...
                {isSubmitting ? <Loader size={14} className="animate-spin" /> : <Send size={14} />} {isSubmitting ? "Judging..." : "Submit"}
              </button>
            </div>
          </div>
          ...
          {results && (
            <div className="bg-dark-900 p-3 rounded-lg overflow-y-auto font-mono text-xs text-gray-300 border border-dark-800 max-h-[180px] space-y-2">
              {results.mode === "run" && (
                <>
                  <div className={`flex items-center gap-2 text-sm font-bold ${results.passed ? "text-neon-green" : "text-red-400"}`}>
                    {results.passed ? <CheckCircle size={16} /> : <XCircle size={16} />}
                    {results.passed ? "Test Passed" : results.status === "error" ? "Error" : "Wrong Answer"}
                  </div>
                  {results.runtime > 0 && (
                  ...
                  {results.output && <div><span className="text-gray-500">Output:</span> {results.output}</div>}
                  {results.expected && !results.passed && <div><span className="text-gray-500">Expected:</span> {results.expected}</div>}
                  {results.error && <div className="text-red-400">{results.error}</div>}
                </>
              )}
              {results.mode === "submit" && (
                <>
                  <div className={`flex items-center gap-2 text-sm font-bold ${results.status === "accepted" ? "text-neon-green" : "text-red-400"}`}>
                    {results.status === "accepted" ? <CheckCircle size={16} /> : <XCircle size={16} />}
                    {results.status === "accepted" ? "Accepted ✓" : `Wrong Answer (${results.passed}/${results.total} passed)`}
                  </div>
                  <div className="flex gap-4 text-[11px] text-gray-500">
                    <span className="flex items-center gap-1"><Cpu size={12} /> Pass Rate: {results.passPercentage}%</span>
                    <span className="flex items-center gap-1"><Clock size={12} /> Avg Runtime: {results.avgRuntime}ms</span>
                  </div>
                  {results.results && results.results.map((r, i) => (
                    <div key={i} className={`flex items-center gap-2 py-1 px-2 rounded ${r.passed ? "bg-neon-green/5" : "bg-red-500/5"}`}>
                      {r.passed ? <CheckCircle size={12} className="text-neon-green" /> : <XCircle size={12} className="text-red-400" />}
                      <span>Test {r.testCase}: Input: {r.input}</span>
                      {!r.passed && r.actual && <span className="text-red-400 ml-auto">Got: {r.actual}</span>}
                      {r.error && <span className="text-red-400 ml-auto text-[10px]">{r.error.substring(0, 80)}</span>}
                    </div>
                  ))}
                </>
              )}
          ...
          {!results && (
            <div className="bg-dark-900 p-3 rounded-lg font-mono text-xs text-gray-500 border border-dark-800">
              {">"} Click Run to test against first test case, or Submit to judge against all test cases...
            </div>
          )}
          ...
          {showVoice && (
            <div className="mt-2 p-4 bg-dark-900 border border-neon-cyan/40 rounded-lg flex flex-col gap-3 animate-fade-in">
              <div className="flex justify-between items-center">
                <h4 className="font-bold text-neon-cyan text-sm flex items-center gap-2"><Mic size={16} /> Explain Your Approach</h4>
              ...
              {(isRecording || transcript) && (
                <textarea className="w-full bg-dark-800 text-sm text-gray-300 p-2 rounded border border-dark-600 h-20 outline-none"
              ...
              {voiceResults && (
                <div className="mt-2 p-3 bg-neon-purple/10 border border-neon-purple/40 rounded-lg text-sm text-gray-200">
                  <p className="font-bold text-neon-magenta mb-1">AI Score: <span className="text-white">{Math.round(voiceResults.score || 0)}%</span></p>
              ...
              {speechData && (
                <div className="p-3 bg-dark-800 border border-dark-700 rounded-lg text-xs space-y-2">
                  <p className="font-bold text-neon-yellow flex items-center gap-1"><Zap size={12} /> Speech Quality</p>
                  <div className="grid grid-cols-3 gap-2">
                    <div className="text-center p-2 bg-dark-900 rounded border border-dark-700">
                      ...
                      <p className={`font-bold ${speechData.clarityScore >= 70 ? "text-neon-green" : "text-neon-yellow"}`}>{speechData.clarityScore}%</p>
                    </div>
                    <div className="text-center p-2 bg-dark-900 rounded border border-dark-700">
                      <p className="text-gray-500 text-[9px]">Structure</p>
                      <p className={`font-bold ${speechData.structureScore >= 60 ? "text-neon-green" : "text-neon-yellow"}`}>{speechData.structureScore}%</p>
                    </div>
                    <div className="text-center p-2 bg-dark-900 rounded border border-dark-700">
                      <p className="text-gray-500 text-[9px]">Fillers</p>
                      <p className={`font-bold ${speechData.totalFillers <= 2 ? "text-neon-green" : "text-red-400"}`}>{speechData.totalFillers}</p>
                    </div>
                  </div>
                  {speechData.suggestions && speechData.suggestions.length > 0 && (
                    <div className="space-y-1 mt-1">
                      {speechData.suggestions.map((s, i) => (
                        <p key={i} className="text-gray-400 text-[11px]">💡 {s}</p>
                      ))}
                    </div>
                  )}
                </div>
