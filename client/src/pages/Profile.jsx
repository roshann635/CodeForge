...
import { Trophy, TrendingUp, Cpu, Activity, Flame, Medal, Award, LogOut } from 'lucide-react';
...
export default function Profile() {
  ...
  if (loading) {
    return <div className="p-8 text-neon-cyan font-mono animate-pulse">Loading Profile...</div>;
  }
    ...
    <div className="h-[calc(100vh-4rem)] overflow-y-auto pr-2 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
         <div className="glass-panel p-5 border border-neon-cyan/30 bg-gradient-to-br from-neon-cyan/10 to-transparent relative overflow-hidden group">
            ...
            <h3 className="text-gray-400 text-xs font-mono uppercase tracking-wider mb-2">Global Rank</h3>
            <p className="text-3xl font-bold text-neon-cyan font-orbitron">{gamify.rankTier}</p>
         ...
         <div className="glass-panel p-5 border border-neon-purple/30 bg-gradient-to-br from-neon-purple/10 to-transparent relative overflow-hidden group">
            ...
            <h3 className="text-gray-400 text-xs font-mono uppercase tracking-wider mb-2">Nodes Solved</h3>
            <p className="text-3xl font-bold text-white font-orbitron">{stats.problemsSolved}</p>
         </div>
         ...
         <div className="glass-panel p-5 border border-neon-yellow/30 bg-gradient-to-br from-neon-yellow/10 to-transparent relative overflow-hidden group">
            ...
            <h3 className="text-gray-400 text-xs font-mono uppercase tracking-wider mb-2">Accuracy Matrix</h3>
            <p className="text-3xl font-bold text-neon-yellow font-orbitron">{stats.accuracy}%</p>
         </div>
         ...
         <div className="glass-panel p-5 border border-red-500/30 bg-gradient-to-br from-red-500/10 to-transparent relative overflow-hidden group">
            ...
            <h3 className="text-gray-400 text-xs font-mono uppercase tracking-wider mb-2">Active Streak</h3>
            <p className="text-3xl font-bold text-red-400 font-orbitron">{gamify.streak?.current || 0} Days</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 border border-dark-700">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            {gamify.badges.length === 0 ? (
              <p className="text-xs text-gray-500 font-mono col-span-3">No badges retrieved. Execute problems in Practice HQ to unlock criteria.</p>
            ) : (
              gamify.badges.map((badge, i) => (
                <div key={i} className="bg-dark-900 border border-dark-600 p-4 rounded-xl flex flex-col items-center justify-center text-center gap-2 transition-all cursor-crosshair">
                  <div className={`w-12 h-12 rounded-full bg-${badge.iconColor}/20 border border-${badge.iconColor}/50 flex items-center justify-center`}>
                    <Award className={`text-${badge.iconColor} w-6 h-6`} />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-gray-300">{badge.name}</p>
        ...
        <div className="glass-panel p-6 border border-dark-700 flex flex-col">
          ...
          <div className="bg-dark-900 border-l-4 border-neon-magenta p-4 rounded-r-lg mb-4">
             <p className="text-sm text-gray-300">Based on your recent Voice Analyzer outputs and test runs, our system detected algorithmic instability in the following zones:</p>
             <ul className="flex flex-wrap gap-2 mt-3">
               {stats.weakAreas && stats.weakAreas.length > 0 ? stats.weakAreas.map((area, i) => (
                 <li key={i} className="text-xs bg-red-500/10 text-red-400 border border-red-500/30 px-2 py-1 rounded font-mono">⚠️ {area}</li>
               )) : <li className="text-xs text-neon-green">All systems optimal</li>}
             </ul>
          </div>
          <p className="text-xs text-gray-500 mb-3 uppercase tracking-wider font-bold">Recommended Inject Path:</p>
          <div className="bg-dark-800 p-3 rounded-lg border border-dark-600 flex items-center justify-between group cursor-pointer hover:border-neon-cyan transition-colors">
            <div>
               <p className="text-sm font-bold text-white group-hover:text-neon-cyan">Review: Graph Traversal (BFS/DFS)</p>
               <p className="text-xs text-gray-400">15 min reading + 2 code executions</p>
            </div>
            <button className="px-3 py-1 bg-neon-cyan/20 text-neon-cyan text-xs font-bold rounded">Execute</button>
          </div>
        </div>
      </div>
    </div>
